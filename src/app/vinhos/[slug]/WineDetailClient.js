...
export default function WineDetailClient({ params }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden mb-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 p-8">
            ...
            <div className="space-y-6">
              <div>
              {wine.winery && (
                  <Link href={`/vinicolas/${wine.winery.slug}`} className="text-2xl text-purple-800 hover:text-purple-800 font-semibold">
                    {wine.winery.name}
                  </Link>
                )}
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {wine.winery.region.country && (
                      <Link href={`/paises/${wine.winery.region.country.slug}`} className="text-blue-600 hover:text-blue-800">
                        <CountryFlag countryName={wine.winery.region.country.name} size="w-4 h-4" /> {wine.winery.region.country.name}
                      </Link>
                    )}
                  </div>
                </div>
        ...
        {/* Related Wines from Winery */}
        {wine.more_wines_from_winery && wine.more_wines_from_winery.length > 0 && (
          <div className="bg-white rounded-lg shadow-md p-6 mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Outros vinhos desta vinícola</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {wine.more_wines_from_winery.map((relatedWine) => (
        ...
        {/* Related Wines from Region */}
        {wine.more_wines_from_region && wine.more_wines_from_region.length > 0 && (
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">
              Outros vinhos da região {wine.winery?.region?.name}
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {wine.more_wines_from_region.map((relatedWine) => (
                <RelatedWineCard key={relatedWine.slug} wine={relatedWine} />
              ))}
            </div>
          </div>
    ...
    <Link href={`/vinhos/${wine.slug}`}>
      <div className="bg-gray-50 rounded-lg p-4 hover:shadow-md transition-shadow duration-300">
        ...
        <div className="space-y-1">
          ...
          {wine.winery && (
            <p className="text-xs text-purple-600 font-medium">{wine.winery.name}</p>
          )}
        </div>
        {wine.rating && (
